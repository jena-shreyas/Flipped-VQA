...
class BaseDataset(Dataset):
    ...
    def _get_padding_id(self, text_id):         # return text_ids upto max_seq_len; if less, pad, if more truncate
        # The issue : if encoded text is truncated, 
        # then prefix_idx, based on orig encoded text length, might go out of range !
        # this fn must only be used for padding, not truncation !
        # INCREASE MAX_SEQ_LENGTH !
        padding_text_id = torch.zeros((len(text_id), self.max_seq_len), dtype=torch.int64) - 1
        # since text_id = (1, 141) (say) - this shape = (1, 128)
        for i, tid in enumerate(text_id):
            padding = self.max_seq_len - len(tid)
            if padding >= 0:
                padding_text_id[i, :len(tid)] = tid
            else:
                padding_text_id[i] = tid[:self.max_seq_len]     # truncate upto max_seq_len = 128
    ...
    def _get_text_token(self, text, answer):
        ...
        # print("VQA")
        # print(len(vqa_id[0]))
        # print("VQA pre idx", vqa_prefix_index)
        # print("VQA vst", vqa_video_start)
        ...
        # print("VAQ")
        # print(len(vaq_id[0]))
        # print("VAQ pre idx", vaq_prefix_index)
        # print("VAQ vst", vaq_video_start)
        ...
        # print(qav_id)
        # print(len(qav_id[0]))
        # print("QAV pre idx", qav_prefix_index)  ## 113
        ...
        # print(type(qav_padding_text_id))  # tensor  
        # print(len(qav_id[0]), qav_padding_text_id.size())  # (1, 128)
        # print(qav_padding_text_id)
        ...
        # print("QAV_label_shape", qav_label.shape)
        # print(qav_label)
        # print("qav_label[0].shape ", qav_label[0].shape)
        # print(qav_prefix_index, qav_prefix_index+self.max_feats)
        # print("LHS shape : ", qav_label[0][qav_prefix_index:qav_prefix_index+self.max_feats].shape)
        # print("rhs SHAPE : ", torch.arange(self.max_feats).shape)
        # print("rhs : ", torch.arange(self.max_feats))
