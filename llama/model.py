...
class Transformer(nn.Module):
    ...
    def forward(self, data, inference=False):
        ...
        for i, layer in enumerate(self.layers[-1 * self.adapter_layer:]):
            # print(i, "VQA")
            # vqa_h = checkpoint(self.custom(layer), vqa_h, start_pos, freqs_cis, mask, adapter[i].half(), vqa_video_start)
            vqa_h = layer(vqa_h, start_pos, freqs_cis, mask, adapter[i].half(), vqa_video_start)

            # print(i, "Cond check")
            if self.args.vaq and not inference:
                # print(i, "VAQ")
                # vaq_h = checkpoint(self.custom(layer), vaq_h, start_pos, freqs_cis, mask, adapter[i].half(), vaq_video_start)
                vaq_h = layer(vaq_h, start_pos, freqs_cis, mask, adapter[i].half(), vaq_video_start)
            
            if self.args.qav and not inference:
                # print(i, "QAV")
                # qav_h = checkpoint(self.custom(layer), qav_h, start_pos, freqs_cis, mask, adapter[i].half(), None)
